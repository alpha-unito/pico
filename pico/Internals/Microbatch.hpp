...
class base_microbatch {
 public:
  ...
  static inline tag_t nil_tag() { return 0; }
   ...
   */
  base_microbatch(tag_t tag__, char *chunk_) : tag_(tag__), chunk(chunk_) {}

  inline tag_t tag() const { return tag_; }

  inline void tag(tag_t tag__) { tag_ = tag__; }

  inline char *payload() const { return chunk; }
...
class Microbatch : public base_microbatch {
 public:
  ...
  inline bool full() const { return allocated == slots; }

  inline bool empty() const { return allocated == 0; }
  ...
  class iterator : public std::iterator<std::input_iterator_tag, DataType> {
   ...
   public:
    ...
    bool operator!=(const iterator &rhs) { return p != rhs.p; }
    DataType &operator*() { return *(DataType *)(p + desc_size); }
  };

  iterator begin() { return iterator(chunk); }

  iterator end() { return iterator(chunk + committed * slot_size); }
