...
TEST_CASE("streaming reduce by key", "streaming reduce by key tag") {
  ...
  auto test_pipe = pico::Pipe()
                       .add(reader)
                       .add(pico::Map<std::string, KV>([](std::string line) {
                         return KV::from_string(line);
                       }))
                       .add(pico::ReduceByKey<KV>([](int v1, int v2) {
                              return v1 + v2;
                            }).window(wsize))  //
                       .add(writer);
  ...
  for (auto kgroup : groups) {
    ...
    for (; it + wsize <= group.end(); it += wsize) {
      // full window
      int wres = 0;
      for (auto wit = it; wit < it + wsize; ++wit) wres += wit->Value();
      expected[k].push_back(KV(k, wres));
    }
    if (it < group.end()) {
      // remaining partial window
      int wres = 0;
      for (; it < group.end(); ++it) wres += it->Value();
      expected[k].push_back(KV(k, wres));
    }
  }
