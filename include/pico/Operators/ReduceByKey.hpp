...
/**
 * Defines a PReduce operator performing a tree reduce function on partitioned
 * input (i.e. reduce by key). The reduce kernel is defined by the user and can
 * be a lambda function, a functor or a function.
 *
...
class ReduceByKey : public UnaryOperator<In, In> {
 ...
 public:
   ...
   */
  std::string name_short() { return "PReduce"; }
  ...
  std::function<V(V&, V&)> kernel() { return reducef; }

 protected:
  ReduceByKey* clone() { return new ReduceByKey(*this); }

  const OpClass operator_class() { return OpClass::REDUCE; }

  bool windowing() const { return win != nullptr; }

  bool partitioning() const { return true; }
